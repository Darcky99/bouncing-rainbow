...
namespace Crosstales.OnlineCheck
{
   ...
   public class OnlineCheck : Crosstales.Common.Util.Singleton<OnlineCheck>
   {
      ...
      private Data.CustomCheck customCheck;
      ...
      public bool ForceWWW
      {
         get
         {
            if (Util.Helper.isWebPlatform || Util.Helper.isWSABasedPlatform)
               return true;
...
#if !CT_DEVELOP
         if (existsCustomCheck && !Util.Helper.isEditor && !string.IsNullOrEmpty(customCheck.URL) && (customCheck.URL.Contains("crosstales.com") || customCheck.URL.Contains("207.154.226.218")))
         {
      ...
      private IEnumerator internetCheck()
      {
         if (!isRunning)
         {
            ...
            if (Util.Config.DEBUG)
               Debug.Log($"[{System.DateTime.Now}] {Util.Constants.ASSET_NAME} running...", this);
            ...
            else
            {
               ...
               {
                  // Microsoft check
                  if (microsoft && !available && !Util.Helper.isAppleBasedPlatform)
                  {
                     if (Util.Constants.DEV_DEBUG)
                  ...
                  // Apple check
                  if (!available && (apple || Util.Helper.isAppleBasedPlatform))
                  {
                     if (Util.Constants.DEV_DEBUG)
         ...
         {
            Debug.LogWarning($"{Util.Constants.ASSET_NAME} already running!", this);
         }
      }
