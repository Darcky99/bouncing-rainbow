...
namespace Crosstales.OnlineCheck.Tool
{
   ...
   [HelpURL("https://www.crosstales.com/media/data/assets/OnlineCheck/api/class_crosstales_1_1_online_check_1_1_tool_1_1_speed_test.html")]
   public class SpeedTest : Crosstales.Common.Util.Singleton<SpeedTest>
   {
      ...
      private Model.Enum.TestSize dataSize = Model.Enum.TestSize.MEDIUM;
      ...
      public Model.Enum.TestSize DataSize
      {
         get => dataSize;
         set => dataSize = value;
      }
      ...
      public bool isPlatformSupported => !Util.Helper.isWebPlatform && !Util.Helper.isWSABasedPlatform;
      ...
      public void Test(Model.Enum.TestSize size)
      {
      ...
      public void Test(string url)
      {
         ...
         if (!isBusy)
            StartCoroutine(test(Model.Enum.TestSize.SMALL, url));
      }
      ...
      private IEnumerator test(Model.Enum.TestSize size, string url)
      {
      ...
      private void speedTest(Model.Enum.TestSize size, string url)
      {
#if (!UNITY_WSA && !UNITY_XBOXONE) || UNITY_EDITOR
         try
         {
            ...
            if (string.IsNullOrEmpty(url))
            {
               switch (size)
               {
                  case Model.Enum.TestSize.SMALL:
                     _url = smallURL;
                     break;
                  case Model.Enum.TestSize.MEDIUM:
                     _url = mediumURL;
                     break;
                  default:
                     _url = largeURL;
            ...
            else
            {
               ...
               using (System.Net.WebClient client = new Util.CTWebClientNotCached())
               {
