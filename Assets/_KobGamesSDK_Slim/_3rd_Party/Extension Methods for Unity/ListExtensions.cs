...
public static class ListExtensions
{
    ...
    #region Koby Extensions
    public static void ForEachIfNotNull<T>(this IEnumerable<T> i_Array, Action<T> i_Action)
	{
        if (i_Array != null)
        {
            for (int i = 0; i < i_Array.Count<T>(); i++)
            {
                if (i_Array.ElementAt(i) != null)
                {
                    i_Action.InvokeSafe(i_Array.ElementAt(i));
                }
                else
                {
                    Debug.LogError("List has a Null Ref. Please double check when deleting objects if it's deleted from the list also.");
                }
            }
        }
	}
