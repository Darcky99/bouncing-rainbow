...
{
    public class Button_ClaimMoney_RV_1 : MonoBehaviour
    {
        ...
        [SerializeField] private float[] m_MultiplierAngles;
        ...
        private void CalculateMoneyToReceiveRV(bool i_Animate)
        {
            ...
            for (int i = 0; i < m_MultiplierAngles.Length - 1; i++)
            {
                if (angleToEvaluate.IsBetweenInclusive(m_MultiplierAngles[i + 1], m_MultiplierAngles[i]))
                {
                    if (m_LastMultiplierPick != i)
                    {
                        m_LastMultiplierPick = i;
                        m_MoneyMultiplier = m_LastMultiplierPick + 2;
                        UpdateMoneyToReceivedRV();
        ...
        {
            for (int i = 0; i < m_MultiplierAngles.Length; i++)
            {
                Gizmos.color = Color.green;
                Gizmos.DrawRay(m_RewardArrowTransform.transform.position, Quaternion.Euler(0, 0, m_MultiplierAngles[i]) * Vector3.up * 1000);
            }
        }
    }
}