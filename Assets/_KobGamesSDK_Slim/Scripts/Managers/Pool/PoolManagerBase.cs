...
namespace KobGamesSDKSlim
{
    ...
    public class PoolManagerBase : Singleton<PoolManager>
    {
        ...
        /*
        [ReadOnly, Header("Pool Objects")] public DictionaryPoolTypeHashSetGameObject ActiveObjects2 = new DictionaryPoolTypeHashSetGameObject();
        [ReadOnly] public DictionaryPoolTypeHashSetGameObject AvailableObjects2 = new DictionaryPoolTypeHashSetGameObject();
        */
        ...
        public GameObject Dequeue(ePoolType i_Type, Vector3? i_Position = null, Quaternion? i_Rotation = null, Vector3? i_Scale = null, Transform i_Parent = null, bool i_Enabled = true)
        {
            var dummyGameObject = DequeueObject(i_Type);
            if (i_Position.HasValue)
            {
                dummyGameObject.transform.position = i_Position.Value;
            }

            if (i_Rotation.HasValue)
            {
                dummyGameObject.transform.rotation = i_Rotation.Value;
            }

            if (i_Scale.HasValue)
            {
                dummyGameObject.transform.localScale = i_Scale.Value;
            }
            if (i_Parent != null)
            {
                dummyGameObject.transform.SetParent(i_Parent);
            }
            
            dummyGameObject.SetActive(i_Enabled);
        ...
        protected GameObject DequeueObject(ePoolType i_type)
        {
